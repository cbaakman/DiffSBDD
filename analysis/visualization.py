...
def save_xyz_file(
    path, one_hot, positions, dataset_info, id_from=0, name="molecule", batch_mask=None
):
    try:
        os.makedirs(path)
    except OSError:
...
def plot_rmsd_distribution(path: Path, rmsd_per_peptide: np.ndarray):
    """
    Generates a simple histogram of the RMSD of each datapoint
    to give an overivew of the variance and range of RMSDs

    Arguments:
      path (Path): path to store the plot
      rmsd_per_peptide (array): array of length equal to the number of
                                 generated datapoints.
    """
    plt.hist(rmsd_per_peptide, bins=100)
    plt.xlabel("RMSD")
    plt.ylabel("Frequency")
    plt.title("Distribution of RMSD in test set")
    plt.savefig(path)

...
class LigandPocketDDPM(pl.LightningModule):
    ...
    def analyze_sample(self, molecules, atom_types, aa_types):
        ...
        # Other basic metrics
        validity, connectivity, uniqueness, novelty = self.ligand_metrics.evaluate(
            molecules
        )[0]
    ...
    def sample_and_analyze_given_pocket(self, n_samples, dataset=None, batch_size=None):
        print(
            f"Analyzing molecule stability given pockets at epoch "
    ...
    def sample_chain_and_save(self, keep_frames):
        ...
        for i in range(n_tries):
            ...
            atom_type_final = torch.argmax(one_hot_final, dim=1).cpu().detach().numpy()

            mol_stable = check_stability(x_final, atom_type_final, self.dataset_info)[0]
        ...
        **kwargs,
        ...
        if self.pocket_representation == "CA":
            pocket_coord = torch.tensor(
                np.array([res["CA"].get_coord() for res in residues]),
                device=self.device,
                dtype=FLOAT_TYPE,
            )
        ...
        pocket_size = torch.tensor(
            [len(pocket_coord)] * n_samples, device=self.device, dtype=INT_TYPE
        )
    ...
    def generate_peptides(
        ...
        **kwargs,
        ...
        """
        Generate peptide given an MHC pocket
        Args:
...
class WeightSchedule:
    def __init__(self, T, max_weight, mode="linear"):
        if mode == "linear":
            ...
            raise NotImplementedError(f"{mode} weight schedule is not " f"available.")
